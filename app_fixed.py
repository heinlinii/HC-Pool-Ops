...
"""
    ...
    try:
        matched_client, matched_property = find_matching_client_and_property(db, raw_text)
        ...
        if matched_client:
            matched_client.notes = (matched_client.notes or "") + memory_note

        if matched_property:
            matched_property.notes = (matched_property.notes or "") + memory_note
    ...
    return templates.TemplateResponse(
        ...
        "capture.html",
        {
            ...
            "matched_property": matched_property.address if matched_property else None,
        }
    )
...
@app.get("/clients/{client_id}/ai-interview")
async def client_ai_interview_page(request: Request, client_id: int):
    db = db_session()
...
@app.post("/clients/{client_id}/ai-interview")
async def client_ai_interview_submit(
    request: Request,
    client_id: int,
    ...
    current_info = f"""
Client ID: {client_obj.id}
Name: {getattr(client_obj, 'name', '')}
Phone: {getattr(client_obj, 'phone', '')}
Email: {getattr(client_obj, 'email', '')}
Address: {getattr(client_obj, 'address', '')}
Notes: {getattr(client_obj, 'notes', '')}
"""
            ...
            messages=[
                {
                    "role": "system",
                    "content": """
You are PoolOps2 AI Client Interview Assistant.

You help a pool contractor clean up saved client and property information.

Use the current client info and the contractor's new interview notes.

Return:
1. Clean updated client summary
2. Suggested fields to update
3. Missing information still needed
4. Access warnings
5. Equipment notes
...
Keep it contractor-ready.
"""
                },
                {
                    "role": "user",
                    "content": f"""
CURRENT SAVED CLIENT INFO:
{current_info}

NEW INTERVIEW NOTES FROM CONTRACTOR:
{interview_notes}
"""
                }
            ]
