...
DEFAULT_THEME = {
    "title": "HEINLIN FIELD OPS CORE 1.0",
    "subtitle": "Built by Hand. Run Like a Machine.",
    "hero_title": "Command Center",
    "hero_subtitle": "Always Having a Damn Good Time. Make the Heinlin name proud. Make this country proud.",
    "background_image": "",
    "calendar_background": "",
...
def ensure_schema():
    ...
    if USE_POSTGRES:
        exec_sql("""CREATE TABLE IF NOT EXISTS poolops2_service_requests (
            id SERIAL PRIMARY KEY, client_id INTEGER, client TEXT DEFAULT '', property_id INTEGER, property TEXT DEFAULT '',
            requested_date TEXT DEFAULT '', request_type TEXT DEFAULT 'Service', status TEXT DEFAULT 'Requested',
            notes TEXT DEFAULT '', created_at TEXT DEFAULT ''
        )""")
    else:
        exec_sql("""CREATE TABLE IF NOT EXISTS poolops2_service_requests (
            id INTEGER PRIMARY KEY AUTOINCREMENT, client_id INTEGER, client TEXT DEFAULT '', property_id INTEGER, property TEXT DEFAULT '',
            requested_date TEXT DEFAULT '', request_type TEXT DEFAULT 'Service', status TEXT DEFAULT 'Requested',
            notes TEXT DEFAULT '', created_at TEXT DEFAULT ''
        )""")
...
def r2_enabled():
    ...
    return boto3 is not None and all(os.environ.get(k, "").strip() for k in required)
...
def r2_public_url(key):
    ...
    return f"/r2-private/{key}"


async def save_upload(file: UploadFile | None, folder: str = "photos"):
    ...
    # Safe local fallback for development only. Render disk is not permanent.
...
def year_grid(year=None, job_rows=None):
    today = date.today()
    year = year or today.year
    jobs = job_rows if job_rows is not None else rows("SELECT * FROM poolops2_jobs ORDER BY id DESC")
    months = []
    ...
    return {"year": year, "months": months, "prev_year": year - 1, "next_year": year + 1}
...
def schedule_year(request: Request, y: int = None):
    u = require_login(request)
    if not u: return login_redirect()
    return templates.TemplateResponse("schedule_year.html", ctx(request, board=year_grid(y, jobs_for_user(u))))
...
def client_portal(request: Request):
    ...
    service_requests = rows("SELECT * FROM poolops2_service_requests WHERE client_id=? OR client=? ORDER BY id DESC", (cid, cname))
    return templates.TemplateResponse("client_portal.html", ctx(request, client=client, properties=props, photos=photos, jobs=jobs, service_requests=service_requests))


@app.post("/client-portal/request")
def client_portal_request(request: Request, property_id: int = Form(0), requested_date: str = Form(""), request_type: str = Form("Service"), notes: str = Form("")):
    u = require_login(request)
    if not u: return login_redirect()
    if not is_client(u) and not is_admin(u):
        return RedirectResponse("/employee", status_code=303)
    ...
    if not client:
        return RedirectResponse("/client-portal", status_code=303)
    prop = one("SELECT * FROM poolops2_properties WHERE id=?", (property_id,)) if property_id else None
    if prop and not property_can_access(u, prop):
        return RedirectResponse("/client-portal", status_code=303)
    exec_sql("INSERT INTO poolops2_service_requests (client_id,client,property_id,property,requested_date,request_type,status,notes,created_at) VALUES (?,?,?,?,?,?,?,?,?)", (client.get("id"), client.get("name", ""), property_id or None, (prop or {}).get("address", ""), requested_date, request_type, "Requested", notes, datetime.now().isoformat(timespec="minutes")))
    return RedirectResponse("/client-portal", status_code=303)
