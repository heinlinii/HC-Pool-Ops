...
# Core 1.0 cloud/photo/weather settings
R2_ACCOUNT_ID = os.environ.get("R2_ACCOUNT_ID", "").strip()
R2_ACCESS_KEY_ID = os.environ.get("R2_ACCESS_KEY_ID", "").strip()
R2_SECRET_ACCESS_KEY = os.environ.get("R2_SECRET_ACCESS_KEY", "").strip()
R2_BUCKET_NAME = os.environ.get("R2_BUCKET_NAME", "").strip()
R2_PUBLIC_URL = os.environ.get("R2_PUBLIC_URL", "").strip().rstrip("/")
TOMORROW_API_KEY = os.environ.get("TOMORROW_API_KEY", "").strip()
DEFAULT_WEATHER_LAT = float(os.environ.get("DEFAULT_WEATHER_LAT", "37.9716"))
DEFAULT_WEATHER_LON = float(os.environ.get("DEFAULT_WEATHER_LON", "-87.5711"))
...
def ensure_schema():
    ...
    if USE_POSTGRES:
        exec_sql("""CREATE TABLE IF NOT EXISTS poolops2_service_requests (
            id SERIAL PRIMARY KEY,
            client_id INTEGER, client TEXT DEFAULT '', property_id INTEGER, property TEXT DEFAULT '', address TEXT DEFAULT '',
            request_type TEXT DEFAULT 'Service', preferred_date TEXT DEFAULT '', status TEXT DEFAULT 'Requested',
...
def r2_is_configured():
    return all([boto3, R2_ACCOUNT_ID, R2_ACCESS_KEY_ID, R2_SECRET_ACCESS_KEY, R2_BUCKET_NAME, R2_PUBLIC_URL])
...
async def save_upload(file: UploadFile | None):
    ...
    # Production path: permanent Cloudflare R2 storage.
    if r2_is_configured():
        key = f"field-ops/{datetime.now().strftime('%Y/%m')}/{name}"
        client = r2_client()
        client.put_object(
            Bucket=R2_BUCKET_NAME,
            Key=key,
            Body=content,
            ContentType=content_type_for(file.filename),
        )
        return f"{R2_PUBLIC_URL}/{key}"

    # Local fallback for development only. Render disk is not permanent.
...
def schedule_year(request: Request, y: int = None):
    u = require_login(request)
    if not u: return login_redirect()
    year = y or date.today().year
    visible_jobs = jobs_for_user(u)
    months = []
    for month in range(1, 13):
        months.append({
            "number": month,
            "name": calendar.month_name[month],
            "days": month_grid(year, month, visible_jobs),
        })
    return templates.TemplateResponse("schedule_year.html", ctx(request, year=year, months=months))
...
@app.post("/employee/gps-ping")
def employee_gps_ping(request: Request, lat: str = Form(""), lng: str = Form("")):
    u = require_login(request)
    if not u or not is_employee(u): return {"ok": False}
    emp = one("SELECT * FROM poolops2_employees WHERE id=?", (u.get("id"),))
    if not emp or not emp.get("clocked_in"):
        return {"ok": False, "reason": "not clocked in"}
    now = datetime.now().isoformat(timespec="minutes")
    exec_sql("UPDATE poolops2_employees SET clock_lat=?, clock_lng=?, last_seen_at=? WHERE id=?", (float(lat) if lat else None, float(lng) if lng else None, now, u.get("id")))
...
@app.post("/client-portal/request-service")
def client_request_service(request: Request, property_id: int = Form(0), request_type: str = Form("Service"), preferred_date: str = Form(""), notes: str = Form("")):
    u = require_login(request)
    if not u: return login_redirect()
    if not is_client(u) and not is_admin(u): return admin_redirect(u)
    client = one("SELECT * FROM poolops2_clients WHERE id=?", (u.get("id"),)) if is_client(u) else None
    if is_admin(u) and not client:
        client = rows("SELECT * FROM poolops2_clients ORDER BY name LIMIT 1")
        client = client[0] if client else None
    if not client:
        return RedirectResponse("/client-portal", status_code=303)
    prop = one("SELECT * FROM poolops2_properties WHERE id=?", (property_id,)) if property_id else None
    if prop and not client_can_access({"id": client.get("id"), "role": "client", "name": client.get("name")}, prop.get("client_id"), prop.get("client")):
        prop = None
    pname = (prop or {}).get("property_name", "")
    address = (prop or {}).get("address", "")
    now = datetime.now().isoformat(timespec="minutes")
    exec_sql("INSERT INTO poolops2_service_requests (client_id,client,property_id,property,address,request_type,preferred_date,status,notes,created_at) VALUES (?,?,?,?,?,?,?,?,?,?)", (client.get("id"), client.get("name", ""), property_id or None, pname, address, request_type, preferred_date, "Requested", notes, now))
    exec_sql("INSERT INTO poolops2_jobs (client,property,address,job_type,status,crew,date,scheduled_start,priority,notes) VALUES (?,?,?,?,?,?,?,?,?,?)", (client.get("name", ""), pname, address, request_type, "Requested", "Unassigned", preferred_date, preferred_date, "Normal", notes))
    return RedirectResponse("/client-portal?requested=1", status_code=303)
...
@app.get("/api/weather")
def api_weather(request: Request, lat: float = None, lon: float = None):
    if not require_login(request):
        return {"ok": False, "error": "login required"}
    lat = lat or DEFAULT_WEATHER_LAT
    lon = lon or DEFAULT_WEATHER_LON
    if not TOMORROW_API_KEY:
        return {"ok": False, "error": "TOMORROW_API_KEY is not set in Render Environment"}
    url = "https://api.tomorrow.io/v4/weather/forecast"
    params = {"location": f"{lat},{lon}", "apikey": TOMORROW_API_KEY, "units": "imperial", "timesteps": "1h,1d"}
    try:
